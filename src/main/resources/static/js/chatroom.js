...
function connectWebSocket() {
    ...
    if (isConnected) {
        ...
        var socket = new SockJS('/bill-chatroom');
        stompClient = Stomp.over(socket);
        stompClient.connect({"uid": getUid()}, function(frame) {
//            console.log('Connected: ' + frame);
            // subscribe 3 topics
            stompClient.subscribe('/topic/ChatroomContact', function(respnose) {
                displayContacts(JSON.parse(respnose.body));
            });
            stompClient.subscribe('/topic/Message', function(respnose) {
...
function connectWs() {
  ...
  stompClient.send("/bill/room1/connect", {}, JSON.stringify(crMessage));
}

function disconnectWs() {
  console.log("disconnectWs...");
  var crMessage = createCrMessage();
  stompClient.send("/bill/room1/disconnect", {}, JSON.stringify(crMessage));
  displayContacts();
}
...
//  console.log("sendCrMessage...");
  ...
  stompClient.send("/bill/room1/sendMessage", {}, JSON.stringify(crMessage));
